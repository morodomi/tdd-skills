---
name: tdd-init
description: INITフェーズ用（Flask/Python）。新しい機能開発のTDDサイクルを開始する。ユーザーの「やりたいこと」から機能名を自動生成し、Overview/Feature docの確認、TDD Cycle docを作成してINITセクションを記録。PLANフェーズへ誘導。ユーザーが「init」「初期化」「新機能」と言った時、または /tdd-init コマンド実行時に使用。Flask/pytest向け。
...
- pyproject.toml または requirements.txt に `flask` が含まれているか

Flaskプロジェクトでない場合：
```
このSkillはFlask/Pythonプロジェクト用です。
現在のディレクトリがFlaskプロジェクトか確認してください。

確認方法:
- app.py または wsgi.py が存在するか
- pyproject.toml/requirements.txt に flask が含まれるか

Flaskプロジェクトのルートディレクトリに移動してから再度実行してください。
```
...
```
3. **品質基準の確認**
   - テストカバレッジ: [プロジェクトの目標]%以上
   - 静的解析: [使用するツール名] Level [レベル]
   - コーディング規約: [規約名]
```

詳細なカスタマイズ方法は、プロジェクトルートの `CLAUDE.md` を参照してください。

## 参考情報

このSkillは、TDDワークフローの**最初のフェーズ**です。
