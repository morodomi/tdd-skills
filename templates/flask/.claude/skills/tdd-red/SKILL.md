---
name: tdd-red
description: REDフェーズ用（Flask/Python）。テストコードのみを作成し、実装コードは一切書かない。ユーザーが「red」「テスト」「テスト作成」と言った時、PLANフェーズ完了後、または /tdd-red コマンド実行時に使用。最新のCycle doc（docs/cycles/YYYYMMDD_hhmm_*.md）のTest ListからTODOを1つWIPに移動してpytestでテストを作成。Flask/pytest向け。
...
最新のCycle docのTest ListからTODO項目を1つずつWIPに移動し、テストを作成します。
...
```python
def test_user_can_update_profile(client, auth_user):
    # Given: ログイン済みユーザーがいる
    client.post('/login', json={'email': auth_user.email, 'password': 'password'})
