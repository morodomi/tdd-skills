---
name: tdd-plan
description: PLANフェーズ用。最新のTDDドキュメント（docs/YYYYMMDD_hhmm_*.md）を検索し、PLANセクションを追記する。実装計画のみ作成し、実装コードは一切書かない。ユーザーが「plan」「計画」「設計」と言った時、または /tdd-plan コマンド実行時に使用。ユーザーと対話しながらPLANセクションを作成。Laravel開発のTDDワークフローで使用。
...
7. **ディレクトリ構造の計画**
   ...
   例:
   - tests/Feature/User/          # ユーザー機能
   - tests/Feature/Product/       # 商品機能
   - tests/Feature/Order/         # 注文機能
   ...
   小規模プロジェクトの場合:
   - tests/Feature/ 直下にファイル配置でも可

   中〜大規模プロジェクトの場合:
   - 機能領域ごとにサブディレクトリを作成（推奨）
   - 10ファイル以上になる場合はサブディレクトリ推奨
   ```
...
- 「Userモデルを作成して」→ 「PLANフェーズでは実装コードを作成できません。まずPLANセクションを完成させましょう」
- 「テストを書いて」→ 「PLANフェーズではテストを作成できません。これはREDフェーズで行います」
- 「マイグレーションファイルを作成して」→ 「PLANフェーズではファイル作成はできません。TDDドキュメントに記載しておきましょう」
