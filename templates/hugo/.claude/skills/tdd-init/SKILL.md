---
name: tdd-init
description: INITフェーズ用（Hugo）。新しいコンテンツ・テーマ開発のTDDサイクルを開始する。ユーザーの「やりたいこと」から機能名を自動生成し、TDD Cycle docを作成してINITセクションを記録。PLANフェーズへ誘導。ユーザーが「init」「初期化」「新機能」「新規ページ」と言った時、または /tdd-init コマンド実行時に使用。Hugo向け。
...
```bash
# Hugoプロジェクトか確認
ls hugo.toml config.toml config.yaml 2>/dev/null
...
Hugoプロジェクトでない場合：
```
このSkillはHugoプロジェクト用です。
現在のディレクトリがHugoプロジェクトか確認してください。

確認方法:
- hugo.toml / config.toml / config.yaml が存在するか
- content/ ディレクトリが存在するか

Hugoプロジェクトのルートディレクトリに移動してから再度実行してください。
```
