---
name: tdd-commit
description: COMMITフェーズ用（Hugo）。REVIEWフェーズ完了後、変更をGitコミットする。Git MCP利用時はMCP経由、未利用時はBashフォールバックで実行。コミットメッセージ自動生成、Co-Authored-By追加。ユーザーが「commit」「コミット」と言った時、REVIEWフェーズ完了後、または /tdd-commit コマンド実行時に使用。Hugo向け。
