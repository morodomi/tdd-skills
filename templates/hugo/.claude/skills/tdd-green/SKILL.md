---
name: tdd-green
description: GREENフェーズ用（Hugo）。REDフェーズで定義したページ構造を実現するための最小限の実装のみを行う。content/にMarkdown、layouts/にテンプレートを作成。ユーザーが「green」「実装」と言った時、REDフェーズ完了後、または /tdd-green コマンド実行時に使用。Hugo向け。
...
REDフェーズで定義したページ構造を実現するための**最小実装**を行うことです。
「動くサイト」を優先し、「綺麗なテンプレート」はREFACTORフェーズに委ねます。

## このフェーズでやること（Hugo）

- [ ] 最新のTDD Cycle ドキュメントを検索する（`ls -t docs/cycles/202*.md | head -1`）
- [ ] Cycle docを読み込み、Test ListのWIP項目を確認する
- [ ] 期待するページ構造（REDで定義）を確認する
- [ ] content/ にMarkdownファイルを作成する
- [ ] layouts/ にテンプレートファイルを作成する
- [ ] `hugo server -D` で表示確認する
- [ ] エッジケース発見時はTest ListのDISCOVEREDセクションに追加（実装はしない）
- [ ] Test ListのWIP項目をDONEに移動する
- [ ] Progress Logに記録（GREEN phase - 作成ファイル、確認結果、発見事項）
- [ ] YAML frontmatterのphaseを"GREEN"に、updatedを更新
- [ ] ビルドが通ることを確認する（`hugo build`）
