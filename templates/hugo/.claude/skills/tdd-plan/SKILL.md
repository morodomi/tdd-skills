---
name: tdd-plan
description: PLANフェーズ用。最新のTDDドキュメント（docs/YYYYMMDD_hhmm_*.md）を検索し、PLANセクションを追記する。実装計画のみ作成し、実装コードは一切書かない。ユーザーが「plan」「計画」「設計」と言った時、または /tdd-plan コマンド実行時に使用。ユーザーと対話しながらPLANセクションを作成。TDDワークフローで使用。
...
6. **テストケース一覧**
   ```
   「テストケースを教えてください」
   - テストファイルのパス（例: tests/[your test framework]/XXXTest.[ext]）
   - テストケース概要
   - テストの種類（統合テスト、単体テスト等）
   ```
...
1. **質問項目の追加**
   - フレームワーク固有の質問（例: ORM設計、APIエンドポイント等）
   - プロジェクト固有の質問（例: マイクロサービス間連携等）
...
3. **テストケースの形式**
   - テストファイル名規則（XXXTest.py, test_XXX.py等）
   - テストディレクトリ構造
...
ここに言語・フレームワーク固有のアーキテクチャセクションを追加してください。

例：
```markdown
### アーキテクチャ設計
- [Your ORM/data access layer]
- [Your migration strategy]
- [Your API/view design]
- [Your UI/routing design]
```
